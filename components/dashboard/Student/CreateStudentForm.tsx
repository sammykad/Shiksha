...
import { studentImporterConfig } from '@/lib/importer/entities/student/config';
...
export default function CreateStudentForm({ leadData, leadId, organizationId, academicYearId }: Props) {
  ...
  // Lead Data
  const nameParts = (leadData?.studentName ?? '').trim().split(' ');
  const firstName = nameParts[0] ?? '';
  const lastName = nameParts.slice(1).join(' ') ?? '';

  // Parse parent name too
  const parentParts = (leadData?.parentName ?? '').trim().split(' ');
  ...
  const form = useForm<z.infer<typeof studentSchema>>({
    ...
    defaultValues: {
      // ✅ Pre-filled from lead if available, empty string if fresh
      firstName,
      lastName,
      middleName: '',
      ...
      emergencyContact: '',
      profileImage: '',
      parents: (parentFirstName || parentLastName)
        ? [
          {
  ...
  return (
    <div className="px-2 space-y-3 my-2"> {
      /* ✅ Visual indicator so admin knows this is from a lead */}
      {leadData && (
        <div className="bg-indigo-50 border border-indigo-200 rounded-lg px-4 py-3 text-sm text-indigo-700 flex items-center gap-2">
          ...
          <span>
            Pre-filled from lead: <strong>{leadData.studentName}</strong>.
            Please fill in remaining fields like grade, section and roll number.
          </span>
        </div>
      )}
      <PageHeader
        title="Student Registration"
        description="Create a comprehensive student profile with all necessary information and documents."
        icon={User}
        actions={
          <>
            <BulkImporter
              config={studentImporterConfig}
                          ...
                          <FormControl>
                            <div className="flex flex-col items-center space-y-4">
                              <div className="relative w-32 h-32 group">
                                <div className="w-full h-full border-2 border-dashed border-slate-300 rounded-full flex items-center justify-center bg-slate-50 hover:bg-slate-100 transition-colors overflow-hidden relative">
                                  <Input
                                    type="file"
                                    accept="image/*"
                                ...
                                {profileImage && !isUploadingImage && (
                                  <button
                                    ...
                                    className="absolute -top-1 -right-1 bg-red-500 text-white p-1 rounded-full shadow-md hover:bg-red-600 transition-colors z-30"
                                  >
