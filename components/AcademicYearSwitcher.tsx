  ...
  return (
    ...
    >
      <div className="flex items-center gap-2.5">
        <div className={cn("flex h-7 w-7 items-center justify-center rounded-md", isCurrent ? "bg-primary/10" : "bg-muted")}>
          {isCurrent
        ...
        <div className="flex flex-col">
          ...
          <span className="text-[10px] text-muted-foreground">
            {isCurrent ? "Latest records & actions apply here" : "Viewing historical data only"}
          </span>
        </div>
      </div>

      {isSelected && (
        <Check className={cn("h-4 w-4 shrink-0", isCurrent ? "text-primary" : "text-amber-500")} />
      )}
    </CommandItem>
  )
}

function HistoricalModeBanner({ viewingYear, currentYear }: { viewingYear: AcademicYear; currentYear: AcademicYear }) {
  return (
    <>
      <CommandSeparator />
      <div className="flex items-start gap-2 bg-amber-50/50 px-3 py-3 dark:bg-amber-950/20">
        ...
        <p className="text-[10px] leading-relaxed text-amber-800/80 dark:text-amber-300/80">
          <span className="font-semibold">Historical Mode:</span> Viewing {viewingYear.name}. New entries still go to{" "}
...
export function AcademicYearSwitcher() {
  ...
  const { years, viewingYear, currentYear, setViewingYear, isViewingPastYear, isPending } = useAcademicYear()
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          ...
          {!isViewingPastYear && (
            <Badge variant="secondary" className="hidden sm:flex h-4 px-1 text-[10px]">
              Current
            </Badge>
          )}

          <ChevronDown className={cn("hidden sm:block h-3.5 w-3.5 text-muted-foreground transition-transform duration-200", open && "rotate-180")} />
        </button>
      </PopoverTrigger>

      <PopoverContent className="w-72 p-0 overflow-hidden shadow-lg" align="end" sideOffset={8}>
        <Command>
          ...
          <CommandList className="max-h-72">
            ...
            {isViewingPastYear && viewingYear && currentYear && (
              <HistoricalModeBanner viewingYear={viewingYear} currentYear={currentYear} />
            )}
          </CommandList>
        </Command>
      </PopoverContent>
