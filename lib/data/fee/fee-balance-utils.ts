...
export function getInFlightPdcAmount(
  payments?: { amount: number | null; status: string }[] | null,
): number {
  return (payments ?? [])
    .filter((p) => p.status === PaymentStatus.CHEQUE_PENDING)
    .reduce((sum, p) => sum + Math.max(p.amount ?? 0, 0), 0);
}

export function getMaxCollectible(
  pendingAmount: number,
  payments?: { amount: number | null; status: string }[] | null,
): number {
  return Math.max(pendingAmount - getInFlightPdcAmount(payments), 0);
}
